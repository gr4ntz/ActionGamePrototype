...
public class ThirdPersonOrbitCamBasic : MonoBehaviour 
{
    ...
    void MainUpdate()
	{
        ...
        //check if target in front or back of player
        if (lob.isLockOn)
        {
            if (lob.target != null)
            {
                var heading = lob.target.position - player.position;
                dot = Vector3.Dot(heading, player.forward);
            }
        }
        ...
        else if (dot<=-1 && lob.isLockOn)
        {
            ...
            angleH = player.rotation.eulerAngles.y;
        }
        else
        {
            cam.position = player.position + camYRotation * smoothPivotOffset + aimRotation * smoothCamOffset;
